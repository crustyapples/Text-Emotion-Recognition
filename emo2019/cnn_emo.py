...
with open('emo2019/tokenizer_pytorch.pkl', 'rb') as file:
    tokenizer = pickle.load(file)

# Load the processed data
with open('emo2019/processed_data_pytorch.pkl', 'rb') as file:
    X_train_pad, X_val_pad, X_test_pad, y_train, y_val, y_test, max_seq_length = pickle.load(file)

# Load the label encoder
with open('emo2019/label_encoder_pytorch.pkl', 'rb') as file:
...
def train_model_with_early_stopping(model, train_loader, val_loader, criterion, optimizer, epochs=10, patience=3):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'saved_models/cnn_emo.pth')  # Save the best model
...
model.load_state_dict(torch.load('saved_models/cnn_emo.pth'))
